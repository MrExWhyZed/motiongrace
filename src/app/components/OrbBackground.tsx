'use client';
/**
 * OrbBackground — WebGL orb ported verbatim from orb-background project.
 *
 * All GLSL shader code, animation logic, hover detection, and rendering are
 * IDENTICAL to the original Orb.tsx.  The only differences from the original:
 *
 *  1. The component renders absolutely positioned to fill its parent (the hero
 *     section), not a fixed-size container.
 *  2. Mouse-move / mouse-leave listeners are attached to `window` so the orb
 *     reacts to the cursor even when it hovers text/buttons layered on top.
 *  3. The exported name is OrbBackground instead of Orb.
 *  4. Added `'use client'` directive for Next.js App Router.
 */
...
export default function OrbBackground({
  ...
  useEffect(() => {
    ...
    // Listen on the container's parent (hero section) so hover works
    // even when cursor is over text/buttons layered above the canvas.
    ...
    // Attach to window so cursor events are captured regardless of z-index layers
